...
class LineRenderer {
    ...
    fun drawLogicalLines(
        ...
        ) // Origin at Target Ball
        canvas.rotate(state.protractorUnit.rotationDegrees) // Canvas rotated

        // Calculate the UNROTATED local position of the Ghost Cue Ball relative to the Target Ball
        // If 0 deg rotation means ghost is along positive Y of target:
        ...
        // Note: The exact (0,Y) or (X,0) depends on how you define 0-degree rotation in ProtractorUnit.
        // Based on ProtractorUnit.protractorCueBallCenter:
        // x = center.x - (distance * sin(angleRad))
        // y = center.y + (distance * cos(angleRad))
        // If angleRad = 0: x = center.x, y = center.y + distance.
        // So, relative to center, it's (0, distance). This matches unrotatedProtractorGhostCueLocalPos.
        ...
        // Log the local position being used for lines
        val pAimLog =
    ...
    // ... (getAngle, drawBankingShotLinesWithReflection, findClosestRailIntersection, getLineSegmentRayIntersection, distanceSq, reflectVector - no changes from previous full file)
    ...
    private fun drawTangentLines(
        ...
        // ghostCuePosInUnitLocalSpace is the position of GhostCue relative to TargetBall (which is at 0,0 on this rotated canvas)
        val dxToTarget = 0f - ghostCuePosInUnitLocalSpace.x // Vector from ghost cue TO target
        ...
        if (magToTarget > 0.001f) {
            ...
            // Tangent direction is perpendicular to the line connecting ghost cue and target
    ...
    private fun drawProtractorAimingAndAngleLines(
        ...
        val originTargetBall = PointF(0f, 0f) // Target Ball is at (0,0) on this rotated canvas
        var lineDrawn = false
