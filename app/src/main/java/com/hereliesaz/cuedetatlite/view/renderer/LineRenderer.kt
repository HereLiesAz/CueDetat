...
class LineRenderer(
    ...
    private fun drawProtractorLines(canvas: Canvas, state: OverlayState) {
        ...
        val actualCueBall = state.actualCueBall

        // Aiming line
        canvas.drawLine(
            protractorUnit.targetBallCenter.x,
            protractorUnit.targetBallCenter.y,
            protractorUnit.protractorCueBallCenter.x,
            protractorUnit.protractorCueBallCenter.y,
            paintCache.greenPaint
        )

        // Shot line
        if (state.showActualCueBall) {
            val dx = protractorUnit.protractorCueBallCenter.x - actualCueBall.center.x
            val dy = protractorUnit.protractorCueBallCenter.y - actualCueBall.center.y
            canvas.drawLine(
