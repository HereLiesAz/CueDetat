...
*   **AR Frame Bridge:** `feature_expert_ar/src/main/java/com/hereliesaz/cuedetat/feature/expert/ar/ArFrameProcessor.kt`
    *   *Responsibility:* Thread-safe bridge from GL-thread ARCore `Frame` objects to `VisionRepository`, using `AtomicReference` so neither thread blocks. Ball detection runs on its own background worker; the GL thread only copies and releases the CPU image.
