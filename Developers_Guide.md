...
3. Rendering Pipeline (Conceptual)
   Event (UI -> ViewModel): A user interaction (e.g., drag) occurs in a Composable or ProtractorOverlayView. An MainScreenEvent is sent to the MainViewModel.

Orchestration (ViewModel):
...
Reduction (ViewModel -> StateReducer): The ViewModel sends the event to the StateReducer. The reducer takes the current ScreenState and the event, and returns a new, updated ScreenState without any side effects.

Derivation (ViewModel -> UpdateStateUseCase): The ViewModel takes the new ScreenState and the current OverlayState, and passes them to the UpdateStateUseCase. This use case calculates all derived data:
...
State Update (ViewModel -> UI): The ViewModel updates its _overlayState Flow with the new state from the use case.
...
Add a UI control to simulate striking the cue ball off-center. This could be a color wheel where dead center is white.

Display a standard set of resulting cue ball paths, color-coded to correspond with the strike spot on the color wheel, to visualize the effects of spin.

Tutorial Enhancements:

Make the tutorial more interactive, highlighting UI elements corresponding to the current step.

Shot Difficulty Analysis:

Provide a "difficulty rating" for the current shot based on cut angle, distance, and proximity to rails/pockets.

"Drills" Mode:

Create a predefined set of common practice shots (e.g., stop shot, follow shot, standard cut angles). The app would guide the user to place the real balls in the correct positions and then show the ideal aiming solution.

Line-of-Aim Sensitivity Control:

Add a user setting to adjust the sensitivity of the aiming rotation gesture to allow for finer, more controlled adjustments.

Logical Positioning Stability:

Ensure that when the view is zoomed or rotated, logical elements (like the ActualCueBall) maintain their exact position relative to the logical table, preventing any perceived drift during transformations.

Theme and UI Toggles:

Fully implement the light/dark mode toggle so that it correctly and dynamically updates the PaintCache and all relevant composables. The default theme will remain dark.

Application Updater Replacement:

Remove any in-app update check logic.

In its place, display the currently installed app version.

Fetch and display the latest available version from the GitHub repository.

Provide a simple, static link to the repository's releases page: https://github.com/HereLiesAz/CueDetat/tree/CueDetatLite.