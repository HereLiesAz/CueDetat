Project Development Guide: Cue D'état Lite
This document outlines the core architecture, concepts, and future direction of the Cue D'état Lite
...
Setting the camera's base rotation to match the anchorOrientation.pitch (to align
with the unlocked view's primary axis at the moment of lock).

Calculating deltas: deltaRoll = currentOrientation.roll - anchorOrientation.roll,
...
Applying counter-rotations to the camera for these deltas (e.g., camera.rotateZ(-deltaRoll)).
...
2. Architectural Model & File Structure
   ...
   com.hereliesaz.cuedetatlitelite/
   ├── data/
...
3. Rendering Pipeline (Conceptual) & Spatial Lock Integration
   Sensor Input (SensorRepository): Provides continuous FullOrientation (yaw, pitch, roll)
   data.

ViewModel (MainViewModel): Receives FullOrientationChanged events and passes them to the
...
Perspective Transformation (Perspective.createPitchMatrix):
...
The camera's base orientation is set using camera.rotateX(anchorOrientation.pitch).
...
Deltas are calculated: deltaRoll = currentOrientation.roll - anchorOrientation.roll,
deltaYaw = currentOrientation.yaw - anchorOrientation.yaw, and
deltaPitch_additional = currentOrientation.pitch - anchorOrientation.pitch.

These deltas are then applied as counter-rotations to the camera (which is already
...
ViewModel (Continued): Emits the fully processed OverlayState (with matrices) to the UI.

Renderer (ProtractorOverlayView via OverlayRenderer):
...
Q. Camera Stabilization: The camera preview stabilization is enabled in CameraBackground.kt by checking for device support and then calling previewBuilder.setPreviewStabilizationEnabled(true). This is a best-effort attempt and may not be supported on all devices.
